<template>
  <div class="bg-tertiary-200" id="home">
    ...
    <header class="bg-primarycontrast-500 sticky z-50 transition-all duration-300" :class="{'shadow-xl': isScrolled, '-top-full': !showHeader, 'top-0': showHeader}">
      <div class="container mx-auto px-4 py-2 flex items-center justify-between max-w-screen-lg">
        <div class="text-2xl font-bold text-primary-500 flex flex-col justify-center cursor-pointer" @click.prevent="scrollToSection('home')">
<!--          <img class="h-20" src="/images/pocketledge-logo-v2-green.svg" alt="">-->
        ...
        <nav class="flex gap-6">
          <a @click.prevent="scrollToSection('price')" class="cursor-pointer text-gray-50 hover:text-primary-500 flex gap-1"><currency-euro-icon class="w-6"></currency-euro-icon><span>Preise</span></a>
          <a @click.prevent="scrollToSection('faq')" class="cursor-pointer text-gray-50 hover:text-primary-500 flex gap-1"><question-mark-circle-icon class="w-6"></question-mark-circle-icon><span>FAQ</span></a>
          <a @click="showContactModal = true" class="cursor-pointer text-gray-50 hover:text-primary-500 flex gap-1">
            <chat-bubble-oval-left-ellipsis-icon class="w-6"></chat-bubble-oval-left-ellipsis-icon><span>Kontakt</span></a>
...
<!--          <img src="/images/pocketledge-logo-v2-white.svg" alt="">-->
        ...
        <div @click="showModal = true" class=" w-fit gradient-button custom-color from-primarycontrast-500 to-primarycontrast-600 ">
      ...
      <section id="testimonials" class="container mx-auto px-4 py-16 max-w-screen-xl">
        ...
        <div class="grid md:grid-cols-3 gap-8">
          ...
          <div class="bg-white rounded-lg shadow p-6 text-center">
            <p class="text-gray-700 italic">"PocketLedge ist ein Gamechanger! Super stabil und perfekt für jede Session!"</p>
            <p class="mt-4 font-bold text-primary-500">- Max aus Berlin</p>
          </div>

          <div class="bg-white rounded-lg shadow p-6 text-center">
            <p class="text-gray-700 italic">"Ich liebe meine Pocketledge, so schnell aufgebaut und passt locker in mein Lastenrad."</p>
            <p class="mt-4 font-bold text-primary-500">- Lisa aus Wien</p>
          </div>

          <div class="bg-white rounded-lg shadow p-6 text-center">
            <p class="text-gray-700 italic">"Der Landi isch volle bärig, aber gibt halt keine Ledge. Jetzt bring ich einfach meine Eigene mit"</p>
    ...
    <div class="bg-primarycontrast-500 w-full">
      <section id="price" class="container mx-auto px-4 py-16 max-w-screen-xl text-center">
        ...
        <!-- Price Card -->
        <div class="max-w-lg mx-auto bg-tertiary-200 rounded-lg shadow-lg p-8 flex flex-col items-center justify-center">
          ...
          <ul class="text-gray-700 text-left space-y-3 mb-6">
            <li class="flex items-center">
              <CheckBadgeIcon class="w-6 text-primary-500"></CheckBadgeIcon> <span class="ml-2">Hochwertige Handarbeit – Made in Austria 🇦🇹</span>
            </li>
            <li class="flex items-center">
              <CheckBadgeIcon class="w-6 text-primary-500"></CheckBadgeIcon> <span class="ml-2">Modulares Design – passt in jede Eurobox (60x40x30)</span>
            </li>
            <li class="flex items-center">
              <CheckBadgeIcon class="w-6 text-primary-500"></CheckBadgeIcon> <span class="ml-2">Sekundenschneller Aufbau – in 1 Minute bereit</span>
            </li>
            <li class="flex items-center">
              <CheckBadgeIcon class="w-6 text-primary-500"></CheckBadgeIcon> <span class="ml-2">Stabile Metallkonstruktion – kein Wackeln, kein Verrutschen</span>
            </li>
          </ul>

          <!-- Order Button -->
          <div @click="showContactModal=true" class="gradient-button custom-color from-primarycontrast-500 to-primarycontrast-600">
            <shopping-cart-icon class="w-6"></shopping-cart-icon>Jetzt bestellen
          </div>
        </div>
      ...
      <section id="faq" class="container mx-auto px-4 py-16 max-w-screen-xl">
        ...
        <div class="max-w-2xl mx-auto space-y-6">
          ...
          <div class="bg-white rounded-lg shadow p-6">
            ...
            <p class="text-gray-700 mt-2">Der Aufbau dauert tatsächlich nur 1 Minute! Dank unseres cleveren Designs kannst du PocketLedge blitzschnell einsatzbereit machen.</p>
          </div>

          <div class="bg-white rounded-lg shadow p-6">
            <h3 class="font-bold text-primary-500">Wie stabil ist PocketLedge?</h3>
            <p class="text-gray-700 mt-2">PocketLedge besteht aus hochwertigem Stahl und bleibt stabil, egal ob du Street- oder Park-Tricks machst.</p>
          </div>

          <div class="bg-white rounded-lg shadow p-6">
            <h3 class="font-bold text-primary-500">Passt PocketLedge wirklich in eine Eurobox?</h3>
            <p class="text-gray-700 mt-2">Ja! Das Design ist so optimiert, dass PocketLedge perfekt in eine Eurobox (60x40x30 cm) passt.</p>
          </div>
    ...
    <div class="bg-tertiary-200">
      <section id="testimonials" class="container mx-auto px-4 py-16 max-w-screen-xl">
        ...
        <div class="mt-8 flex flex-col items-center justify-center gap-6">
          ...
          <div @click="showContactModal=true" class="gradient-button custom-color from-primarycontrast-500 to-primarycontrast-600">
            <photo-icon class="w-6"></photo-icon>Schick uns dein Bild!
          </div>
        </div>
      </section>
    </div>
...
import { PlayPauseIcon, ShoppingCartIcon, CurrencyEuroIcon, QuestionMarkCircleIcon, ChatBubbleOvalLeftEllipsisIcon, PhotoIcon} from '@heroicons/vue/24/outline'
